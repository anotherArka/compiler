...
instance Eq Term where
  ...
  (Lambda s this) == (Lambda t that) = (this == that) && (s == t)
...
show_helper level (Inr x t) = "(Inr" ++ (show_helper level x) ++ ")"
show_helper level (Inl x t) = "(Inl" ++ (show_helper level x) ++ ")"
show_helper level (Pair x y) = "(" ++ (show_helper level x) ++ "," ++ (show_helper level y) ++ ")"
show_helper level (Lambda t inside) = "/" ++ "#x" ++ (show level) ++ 
  ":" ++ (show t) ++
  "." ++ (show_helper (level + 1) inside)
show_helper level (App x y) = "(" ++ (show_helper level x) ++ " " ++ (show_helper level y) ++ ")"
-- show_helper level (Void t) = "void"
